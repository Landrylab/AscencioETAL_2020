...
lmts <- c(0.15,1)
dt$subcomplex <- factor(dt$subcomplex, levels = c("regulatory particle","core complex"))
pC <- ggbarplot(filter(dt, attenuation > -100), 
              ...
              geom_hline(yintercept = 0.5, linetype="dashed",
                color = "gray66",alpha = 0.3, size=1.5) +  
                geom_text(mapping = aes(x = 10, y = 0.52, label = "complete attenuation"), 
                          family="sans",size = 3)  +
                xlab("")+
...
# PANEL D:  Compare the attenuation in different components and subcomplexes --------
anodata <- dt  %>% dplyr::filter(attenuation > 0) %>%
                   select(c(1,5,10))
...
pw <- filter(pwc, p.adj < 0.05)
dt$component <- factor(dt$component, levels = c("alpha.ring","beta.ring","ATPase","non.ATPase"))
cpalet <- c("#F6AFA5", "#ED553D", "#98DCD4", "#3CAEA3")
pD <- ggplot(filter(dt, attenuation > 0), aes(component, attenuation)) +
             geom_jitter(aes(color = component), 
              position = position_jitter(0.2), 
              size = 2, alpha = 0.8) + 
               stat_summary(fun.y = mean, geom = "errorbar", aes(ymax = ..y.., ymin = ..y.., color = component),
               width = .75, size = 2)+
              scale_color_manual(values =  cpalet)+
              xlab("")+ ylab("Attenuation score")+
               stat_pvalue_manual(pw, bracket.size = 0.3, label =  "p.adj.signif",
...
# panel E: PRE7 colony size scatter  -----------------------------------------------------------------
...
pE <- ggscatter(f,x="mean_control", y="mean_dup",color="type",
               alpha = 0.7, size = 2,
...
fplot2 <- ggdraw() +
  draw_plot(pF, x = 0.5, y = 0, width = 0.45, height = 0.32) +
  draw_plot(pD, x = 0.56, y = 0.3, width = .4, height = 0.37) +
  draw_plot(pE, x = 0.03, y = 0, width = 0.45, height = 0.32) +
  draw_plot(pC, x = 0, y = 0.3, width = .55, height = 0.37) +
  ...
  draw_plot_label(label = c("A", "B", "C", "D", "E", "F"), size = 15, 
                  x = c(0,0.39,0,0.55,0,0.5), 
                  y = c(1,1,0.65,0.65,0.33,0.33))
...
#save as pdf
pdf(file = "Figures/Figure3new.pdf", width = 8, height = 10)
fplot2
dev.off()
